...
optimal and only the constant is lost. \Cref{sec:tightness} decomposes that
constant.
...
supremum norm (orange) is valid but can be vacuous; paying for it in $L^2$
against a richer polynomial class (green) is valid and informative, and improves
as the fitted class grows. The dashed line is the true defect, obtained by
Monte-Carlo integration of the analytic bracket.}
...
requires --- at nominal $0.5$ the observed coverage is $1.0$ --- because
\eqref{eq:U} bounds numerator and denominator separately over the whole
ellipsoid. Panel (b) constructs the worst case deliberately: a system whose true
defect is $(1+\tau)\delta$ for $\tau$ as small as $0.02$, observed at the sample
size that puts the median certified bound exactly at $\delta$. The
false-certification rate remains $0$ at every $\alpha$ we tried.
...
validity, and at $\alpha = 0.5$ in the boundary configuration it does certify
falsely, at rate $0.18$. Its intervals are $18\%$--$21\%$ narrower than ours.
That is the price of exactness on this problem: roughly a fifth of the width.
...
sandwich covariance \citep{white1980heteroskedasticity}. Coverage never falls
below nominal in any of the $24$ settings. The certificate is not fragile to the
shape of the error distribution, because it depends on the errors only through a
confidence ellipsoid for a least-squares estimator.

The assumption that does bind is that the \emph{states} are observed without
error. When derivatives are estimated by central differences from noisy states,
the regressors themselves are noisy; the estimator is biased toward zero and no
covariance correction repairs it. In our runs this bias happens to act
conservatively --- it inflates the defect, so coverage held at $1.0$ while
detection collapsed from $0.98$ to $0.06$ --- but that is a property of this
attenuation, not a guarantee. For that regime the honest options are the weak or
...
$n = 6$. One certificate costs $0.15$ to $0.30$ seconds at these sizes, dominated
by the $O(Q^3)$ eigendecomposition inside the trust-region solves; the timings
were taken on a heavily loaded machine and should be read as upper bounds.
